...
export async function GET(req: NextRequest) {
  ...
  try {
      ...
      for (const roomType of roomTypes) {
       ...
       const todayBookings = await prisma.booking.findMany({
        ...
        for (const room of typeRooms) {
          ...
          if (unsoldPercent >= settings.middleUnsold && settings.middleDiscount > 0) {
            finalPrice = room.price - (room.price * settings.middleDiscount) / 100;
          } else if (bookedPercent >= settings.middleBooked && settings.middleMarkup > 0) {
            finalPrice = room.price + (room.price * settings.middleMarkup) / 100;
          }
    ...
    return NextResponse.json({ success: true, message: "Middle hours slot processed!" });
  } catch (error) {
    console.error(error);
