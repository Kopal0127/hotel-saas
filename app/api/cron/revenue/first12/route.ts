...
export async function GET(req: NextRequest) {
  try {
    ...
    for (const hotel of hotels) {
      ...
      for (const roomType of roomTypes) {
        ...
        for (const room of typeRooms) {
          ...
          if (unsoldPercent >= settings.first12Unsold && settings.first12Discount > 0) {
            finalPrice = room.price - (room.price * settings.first12Discount) / 100;
          } else if (bookedPercent >= settings.first12Booked && settings.first12Markup > 0) {
            finalPrice = room.price + (room.price * settings.first12Markup) / 100;
          }
    ...
    return NextResponse.json({ success: true, message: "First 12hrs slot processed!" });
  } catch (error) {
    console.error(error);
