...
export default function ProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      ...
      if (res.ok) { setUser(data.user); } else { router.push("/login"); }
    } catch (error) {
  ...
  const handlePasswordChange = async () => {
    if (!passwordForm.currentPassword) { showToast("Current password daalna zaroori hai!", "error"); return; }
    if (!passwordForm.newPassword) { showToast("Naya password daalna zaroori hai!", "error"); return; }
    if (passwordForm.newPassword.length < 6) { showToast("Password kam se kam 6 characters ka hona chahiye!", "error"); return; }
    if (passwordForm.newPassword !== passwordForm.confirmPassword) { showToast("Dono passwords match nahi kar rahe!", "error"); return; }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-8 py-6 md:py-10">
        <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-6 md:mb-8">My Profile</h2>
        ...
        {activeTab === "profile" && (
          <div className="bg-white rounded-2xl p-5 md:p-8 shadow-sm border border-gray-100">
            <div className="flex items-center gap-4 mb-6 md:mb-8">
              <div className="w-14 h-14 md:w-16 md:h-16 bg-blue-100 rounded-full flex items-center justify-center text-xl md:text-2xl font-bold text-blue-600">
                {user.name?.charAt(0)?.toUpperCase() || "U"}
              </div>
              <div>
                <h3 className="text-base md:text-lg font-bold text-gray-900">{user.name}</h3>
        ...
        {activeTab === "password" && (
          <div className="bg-white rounded-2xl p-5 md:p-8 shadow-sm border border-gray-100">
            <h3 className="text-base md:text-lg font-semibold text-gray-900 mb-5 md:mb-6">Password Change Karo</h3>
