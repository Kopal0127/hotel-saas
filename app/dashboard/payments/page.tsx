  ...
  function generateInvoice(payment: any) {
    const invoiceHTML = `
      <!DOCTYPE html><html><head><style>
        ...
        <div class="header">
          <div><div class="logo">HotelPro</div><div style="color:#666;margin-top:8px;">Hotel Management System</div></div>
          <div style="text-align:right">
            <div style="font-size:20px;color:#666;">INVOICE</div>
            <div style="font-size:14px;color:#999;">#INV-${payment.id.substr(0, 8).toUpperCase()}</div>
        ...
        <table><thead><tr><th>Description</th><th>Amount</th><th>Status</th></tr></thead>
        <tbody><tr><td>Hotel Booking Payment</td><td>₹${payment.amount.toLocaleString('en-IN')}</td>
        <td><span class="status">${payment.status}</span></td></tr></tbody></table>
        ...
        <div class="footer"><p>Thank you for choosing HotelPro!</p><p>This is a computer generated invoice.</p></div>
      </body></html>`
    const printWindow = window.open('', '_blank')
    if (printWindow) { printWindow.document.write(invoiceHTML); printWindow.document.close(); printWindow.print() }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-8 py-6 md:py-10">
        <div className="mb-6 md:mb-8">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800">💳 Payment Gateway</h1>
          <p className="text-gray-500 mt-1 text-sm">Payments manage karo aur invoices generate karo</p>
        </div>
            ...
            <div className="overflow-x-auto">
              <table className="w-full min-w-[600px]">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="text-left px-4 md:px-6 py-3 text-xs md:text-sm font-medium text-gray-600">Guest</th>
                    <th className="text-left px-4 md:px-6 py-3 text-xs md:text-sm font-medium text-gray-600">Amount</th>
                    <th className="text-left px-4 md:px-6 py-3 text-xs md:text-sm font-medium text-gray-600">Status</th>
                    <th className="text-left px-4 md:px-6 py-3 text-xs md:text-sm font-medium text-gray-600">Date</th>
                    <th className="text-left px-4 md:px-6 py-3 text-xs md:text-sm font-medium text-gray-600">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {payments.map(payment => (
                    <tr key={payment.id} className="border-t hover:bg-gray-50">
                      <td className="px-4 md:px-6 py-4">
                        ...
                        <p className="text-xs text-gray-500 hidden md:block">{payment.guestEmail}</p>
                      </td>
                        ...
                        <div className="flex gap-2">
                          {payment.status === 'PENDING' && (
                            <button onClick={() => markPaid(payment.id)}
                              className="text-xs px-2 md:px-3 py-1 bg-green-600 text-white rounded-lg hover:bg-green-700">
                              ✓ Paid
                            </button>
                          )}
                          <button onClick={() => generateInvoice(payment)}
                            className="text-xs px-2 md:px-3 py-1 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                            🖨️ Invoice
                          </button>
                        </div>
                      </td>
