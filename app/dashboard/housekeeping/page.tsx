          ...
          {requests.map((req) => (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  <h3 className="text-lg font-bold text-gray-800">🛏️ Room #{req.roomNumber}</h3>
                  <p className="text-sm text-gray-500">{req.requestType}</p>
                </div>
                <div className="flex flex-col items-end gap-1">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${statusColors[req.status]}`}>
                    {req.status === "PENDING" ? "🔔 Pending" : req.status === "IN_PROGRESS" ? "🧹 In Progress" : "✅ Done"}
                  </span>
              ...
              {req.notes && (
                <p className="text-xs text-gray-500 mb-3 bg-gray-50 rounded p-2">📝 {req.notes}</p>
              )}
